...
class Node(object):
    __slots__ = ('neighbors', '_index',
                 'orders', 'drivers',
                 'order_num', 'idle_driver_num',
    ...
    def simple_order_assign_broadcast_update(self, city, neighbor_node_reward):
        ...
        pending = self.order_num
        shortages = []  # [(缺口数量, neigh_initially_had_car)]
        # 遍历每个邻居
        for neigh in self.neighbors:
            if neigh is None:
                continue

            init_idle = city.neighbor_idle_snapshot.get(neigh.get_node_index(), 0)
            avail = neigh.idle_driver_num
            if init_idle == 0 and avail == 0:
                # 本邻居从未有车，跳过
                continue
            ...
            short = need - given
            if short > 0 and init_idle > 0:
                # 本邻居起初有车，但已被抢光造成的缺口
                shortages.append((short, True))

            if given > 0:
                # 真正派单
                rr = self.utility_assign_orders_neighbor(city, neigh, given)
                reward += rr
                neighbor_node_reward[neigh.get_node_index()] += rr
        ...
        # 冲突结算
        total_shortage_init = sum(s for s, had in shortages if had)

        if pending == 0:
            # 所有缺口最终都被补上 → 全部记为 same_grid_contention
            city.metrics.add_same_grid_contention(total_shortage_init)
        else:
            # 部分补上视为 same_grid_contention，其余视为 unserved_demand
            conflicts_add = min(pending, total_shortage_init)
            samegrid_add = total_shortage_init - conflicts_add

            if samegrid_add > 0:
                city.metrics.add_same_grid_contention(samegrid_add)
            if conflicts_add > 0:
                city.metrics.add_unserved_demand(conflicts_add)
        # 删除已完成订单
