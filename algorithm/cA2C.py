...
class Estimator:
    ...
    def __init__(self,
        ...
        with tf.compat.v1.variable_scope(scope):
            ...
            # self.loss_gradients = tf.compat.v1.gradients(self.value_loss, tf.compat.v1.trainable_variables(scope=scope))
                                           # tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.GLOBAL_VARIABLES, scope=self.scope))

        # Summaries for Tensorboard
        self.summaries = tf.compat.v1.summary.merge([
            tf.compat.v1.summary.scalar("value_loss", self.value_loss),
            tf.compat.v1.summary.scalar("value_output", tf.compat.v1.reduce_mean(self.value_output)),
            # tf.compat.v1.summary.scalar("gradient_norm_policy", tf.compat.v1.reduce_sum([tf.compat.v1.norm(item) for item in self.loss_gradients]))
        ])

        self.policy_summaries = tf.compat.v1.summary.merge([
            tf.compat.v1.summary.scalar("policy_loss", self.policy_loss),
            tf.compat.v1.summary.scalar("adv", tf.compat.v1.reduce_mean(self.adv)),
    ...
    def _build_value_model(self):
        ...
        self.state = X = tf.compat.v1.placeholder(shape=[None, self.state_dim], dtype=tf.compat.v1.float32, name="X")

        # The TD target value
        self.y_pl = tf.compat.v1.placeholder(shape=[None, 1], dtype=tf.compat.v1.float32, name="y")

        self.loss_lr = tf.compat.v1.placeholder(tf.compat.v1.float32, None, "learning_rate")

        # 3 layers feed forward network.
        l1 = fc(X, "l1", 128, act=tf.compat.v1.nn.relu)
        l2 = fc(l1, "l2", 64, act=tf.compat.v1.nn.relu)
        l3 = fc(l2, "l3", 32, act=tf.compat.v1.nn.relu)
        # l1 = tf.compat.v1.layers.dense(X, 1024, tf.compat.v1.nn.sigmoid, trainable=trainable)
        # l2 = tf.compat.v1.layers.dense(l1, 512, tf.compat.v1.nn.sigmoid, trainable=trainable)
        # l3 = tf.compat.v1.layers.dense(l2, 32, tf.compat.v1.nn.sigmoid, trainable=trainable)
        self.value_output = fc(l3, "value_output", 1, act=tf.compat.v1.nn.relu)

        # self.losses = tf.compat.v1.square(self.y_pl - self.value_output)
        self.value_loss = tf.compat.v1.reduce_mean(tf.compat.v1.squared_difference(self.y_pl, self.value_output))
    ...
    def _build_mlp_policy(self):
        ...
        self.policy_state = tf.compat.v1.placeholder(shape=[None, self.state_dim], dtype=tf.compat.v1.float32, name="P")
        self.ACTION = tf.compat.v1.placeholder(shape=[None, self.action_dim], dtype=tf.compat.v1.float32, name="action")
        self.adv = tf.compat.v1.placeholder(shape=[None, 1], dtype=tf.compat.v1.float32, name='advantage')
        self.neighbor_mask = tf.compat.v1.placeholder(shape=[None, self.action_dim], dtype=tf.compat.v1.float32, name="neighbormask")
        # this mask filter invalid actions and those action smaller than current grid value.

        l1 = fc(self.policy_state, "l1", 128, act=tf.compat.v1.nn.relu)
        l2 = fc(l1, "l2", 64, act=tf.compat.v1.nn.relu)
        l3 = fc(l2, "l3", 32, act=tf.compat.v1.nn.relu)

        self.logits = logits = fc(l3, "logits", self.action_dim, act=tf.compat.v1.nn.relu) + 1  # avoid valid_logits are all zeros
        self.valid_logits = logits * self.neighbor_mask

        self.softmaxprob = tf.compat.v1.nn.softmax(tf.compat.v1.log(self.valid_logits + 1e-8))
        self.logsoftmaxprob = tf.compat.v1.nn.log_softmax(self.softmaxprob)

        self.neglogprob = - self.logsoftmaxprob * self.ACTION
        self.actor_loss = tf.compat.v1.reduce_mean(tf.compat.v1.reduce_sum(self.neglogprob * self.adv, axis=1))
        self.entropy = - tf.compat.v1.reduce_mean(self.softmaxprob * self.logsoftmaxprob)

        self.policy_loss = self.actor_loss - 0.01 * self.entropy

        self.policy_train_op = tf.compat.v1.train.AdamOptimizer(self.loss_lr).minimize(self.policy_loss)
