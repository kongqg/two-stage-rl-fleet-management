...
def fc(x, scope, nh, act=tf.nn.relu, init_scale=1.0):
    with tf.compat.v1.variable_scope(scope):
        nin = x.get_shape()[1]
        # 替换 tf.contrib.layers.xavier_initializer 为 tf.compat.v1.keras.initializers.glorot_uniform
        initializer = tf.compat.v1.keras.initializers.glorot_uniform(seed=0)
        w = tf.compat.v1.get_variable("w", [nin, nh], initializer=initializer)
...
def collision_action(action_tuple):
    # 如果你有一条从格子 A→B 的派单，又有一条从 B→A 的派单，就把它算作一次“冲突”
    count = 0
    action_set = set(())
